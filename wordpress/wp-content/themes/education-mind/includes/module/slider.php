...
if ( ! function_exists( 'education_mind_add_featured_slider' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function education_mind_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'education_mind_filter_slider_status', true );
		...
		$slider_details = apply_filters( 'education_mind_filter_slider_details', $slider_details );

		if ( empty( $slider_details ) ) {
			return;
		}

		education_mind_render_featured_slider( $slider_details );

	}

endif;

add_action( 'education_mind_action_before_content', 'education_mind_add_featured_slider', 5 );

if ( ! function_exists( 'education_mind_check_slider_status' ) ) :
	 ...
	 */
	function education_mind_check_slider_status( $input ) {

		$input = false;

		// Slider status.
		$featured_slider_status = education_mind_get_option( 'featured_slider_status' );
...
add_filter( 'education_mind_filter_slider_status', 'education_mind_check_slider_status' );

if ( ! function_exists( 'education_mind_get_slider_details' ) ) :
	 ...
	 */
	function education_mind_get_slider_details( $input ) {

		$featured_slider_type           = education_mind_get_option( 'featured_slider_type' );
		$featured_slider_number         = education_mind_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = education_mind_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = education_mind_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							...
							$slides[ $cnt ]['excerpt'] = education_mind_get_the_excerpt( apply_filters( 'education_mind_filter_slider_caption_length', 30 ), $post );
...
add_filter( 'education_mind_filter_slider_details', 'education_mind_get_slider_details' );

if ( ! function_exists( 'education_mind_render_featured_slider' ) ) :
	 ...
	 */
	function education_mind_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}

		$featured_slider_transition_effect   = education_mind_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = education_mind_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = education_mind_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = education_mind_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = education_mind_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = education_mind_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = education_mind_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = education_mind_get_option( 'featured_slider_transition_delay' );
